...
const VisitaModal = ({
    ...
    const [formData, setFormData] = useState({
        ...
        fechaProgramada: "",
        idTipoServicio: "",
        idEstado: 1, // por defecto “Registrado”
    });
    ...
    useEffect(() => {
        if (editData) {
            setFormData({
                ...
                idEstado: editData.idEstado || editData.estado?.id || 1,
            });
        } else {
            setFormData({
                idCliente: "",
    ...
    const loadSelects = async () => {
        try {
            const [empleadosRes, usuariosRes, clientesRes, tiposRes, estadosRes] = await Promise.all([
                ...
                axios.get(`${apiVisita}/visita/tipoServicio`),
            ...
            setTecnicos(empleados.filter((e) => tecnicosIds.includes(e.id)));
                        ...
                        >
                            {estados.map((e) => (
                                <MenuItem key={e.id} value={e.id}>
                                    {typeof e.tipo === "string" ? e.tipo : e.tipo?.nombre || `Estado ${e.id}`}
                                </MenuItem>
                            ))}
                        </TextField>
                    </Grid>
